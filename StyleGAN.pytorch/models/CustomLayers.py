...
class EqualizedLinear(nn.Module):
    ...
    def __init__(self, input_size, output_size, gain=2 ** 0.5, use_wscale=False, lrmul=1, bias=True):
        super().__init__()
        # w' = w / sqrt(fan_in / 2) 或 w' = w * sqrt(2/fan_in) 
...
# To mitigate, the generator blurs the layer, by convolving with the simplest possible smoothing kernel.
...
class Truncation(nn.Module):
    ...
    def forward(self, x):
        assert x.dim() == 3
        interp = torch.lerp(self.avg_latent, x, self.threshold)  # 线性插值
        do_trunc = (torch.arange(x.size(1)) < self.max_layer).view(1, -1, 1).to(x.device)
        return torch.where(do_trunc, interp, x) # 如果condition满足，则返回值对应位置取x，否则取y
